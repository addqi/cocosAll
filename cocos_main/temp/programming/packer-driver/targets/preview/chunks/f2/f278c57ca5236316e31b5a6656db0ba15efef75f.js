    ...
    execute: function () {
      ...
      _export("Solider", Solider = (_dec = ccclass('Solider'), _dec(_class = class Solider extends Component {
        ...
        init(index, team, pos, cardInfo) {
          ...
          var _model = this.node.children[0];

          var _fightModel = _model.getComponent(_crd && SoliderModel === void 0 ? (_reportPossibleCrUseOfSoliderModel({
            error: Error()
        ...
        lateUpdate(dt) {
          if (this._target) {
            var targetPos = this._target.getPosition().clone();

            var offset = targetPos.subtract(this.node.getPosition().clone());
            var diss = Math.floor(offset.length()); // console.log("diss距离是:"+diss);
