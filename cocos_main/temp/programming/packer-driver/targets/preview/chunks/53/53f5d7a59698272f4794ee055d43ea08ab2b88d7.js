    ...
    execute: function () {
      ...
      _export("FightManager", FightManager = (_dec = ccclass('FightManager'), _dec(_class = (_class2 = class FightManager extends Component {
        ...
        touchStart(event) {
          ...
          if (camera && event) {
            var ray = camera.screenPointToRay(event.getLocation().x, event.getLocation().y);
        ...
        touchEnd(touch) {
          ...
          if (camera && touch) {
            var ray = camera.screenPointToRay(touch.getLocationX(), touch.getLocationY());
            console.log("ray:" + ray);
            ...
            if (ray) {
              ...
              var pos = v3(raycastClosestResult.hitPoint.x, 0, raycastClosestResult.hitPoint.z);
              console.log("pos:" + pos);
              var cardList = (_crd && CardManager === void 0 ? (_reportPossibleCrUseOfCardManager({
                error: Error()
              }), CardManager) : CardManager).instance.chooseCards;
              var cardInfo = this.getSoliderInfo(cardList);
        ...
        addSolider(team, pos, cardInfo) {
          ...
          resources.load("prefab/solider/solider", (err, prefab) => {
            ...
            var soliderScript = node.getComponent(_crd && Solider === void 0 ? (_reportPossibleCrUseOfSolider({
              error: Error()
            }), Solider) : Solider);
        ...
        getSoliderInfo(cardarr) {
          var cardInfo = cardarr[0];
          var obj = (_crd && ConfigManager === void 0 ? (_reportPossibleCrUseOfConfigManager({
            error: Error()
