...
> 当前重构约束：
> - 运行时只读取 `json` 配置
> - Excel 只是上游策划源，后续通过自动化脚本同步到 JSON
> - 现阶段优先保证程序可直接改 JSON、快速验证效果
>
> 对应重构文档：
...
这章原始设计把“技能 = Buff”“升级 = SkillManager 映射”写得太死了。这个思路能跑，但扩展到多职业、主动技能、命中效果组合时会迅速变脏。

当前阶段的目标不是推翻 Buff 系统，而是把职责重新切干净：

- `Buff` 只负责**持续状态**
- `HitEffect` 只负责**命中后附加行为**
- `Skill` 只负责**主动施法入口**
- `Upgrade` 只负责**分发效果 / 装备技能 / 修改行为**
- 运行时统一只读 JSON，方便程序直接改表测试

后续新增内容时，优先遵守这条规则：

1. 纯数值变化：走 `Buff`
2. 命中后副作用：走 `HitEffect`
3. 需要按键释放 / 冷却：走 `Skill`
4. 肉鸽三选一获得的长期增益：走 `Upgrade`

不要再把“升级 / Buff / 技能”混成一个大杂烩。
