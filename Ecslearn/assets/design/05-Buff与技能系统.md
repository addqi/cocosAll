...
> 前置阅读：[02-角色与战斗系统](./02-角色与战斗系统.md)、[04-肉鸽核心机制](./04-肉鸽核心机制.md)
...
### Step 1：属性注册（预计 30 分钟）
- [ ] 为玩家 Entity 创建 `GeneralPropertyMgr`
- [ ] 注册上表中的所有属性
- [ ] CombatSystem / MovementSystem 从属性管理器读取最终值
- **验收**：`getFinalValue('atk')` 返回正确基础值

### Step 2：第一个 Buff（预计 1 小时）
- [ ] 实现 `AtkPowerBuff`（最简单的加法 Buff）
- [ ] 注册到 `BuffFactory`
- [ ] 手动 `addBuff` 后验证攻击力变化
- **验收**：加 Buff 后 `getFinalValue('atk')` 增加了
...
> 下一章：[06-UI与数据驱动](./06-UI与数据驱动.md) —— 让玩家看到发生了什么。
