...
export class PalettePanel extends Component {
    ...
    setup(
        ...
        const vSpc = this._vSpc = options?.itemSpacing ?? this.itemSpacing;
        ...
        const viewW = this._viewW = view.getVisibleSize().width;
        ...
        for (let s = 0; s < POOL_SIZE; s++) {
            const slot = this._createSlot(spriteLayer, labelLayer, itemFrame, s);
    ...
    hitTest(colorPageIndex: number, localX: number, localY: number): number {
        ...
        for (let i = start; i < end; i++) {
            ...
            const cx = this._colX(0, col);
            const cy = this._rowY(row);
    ...
    private _colX(pageCenterX: number, col: number): number {
    ...
    private _createSlot(
        spriteLayer: Node, labelLayer: Node, frame: SpriteFrame, idx: number,
    ): PaletteSlot {
        const iw = this._iw;
        const ih = this._ih;
        ...
        const labNode = new Node(`Lab_${idx}`);
        ...
        return { ringNode, bgNode, labNode, ringSp, bgSp, lab, boundIndex: -1 };
    }
    ...
    private _bindSlot(slot: PaletteSlot, index: number): void {
        ...
        const x = this._colX(page * this._viewW, col);
